...
///Kullanıpta Star Basmayana Hakkım Helal Değildir////

client.on("message", function (msg) {
	// if message begins with "ping"
	if (msg.content.indexOf(".tag") === 0) {
		// send a message to the channel the ping message was sent in.
		msg.channel.send("İsim Taglarımız: `Røse, Rose, ¿`\nEtiket Tagımız `#0105`");
...
client.on("guildMemberAdd", member => {
    let sunucuid = "934514804494831717"; 
    let tag = "Røse";
    let rol = "937013980243849226"; 
  ...
  member.roles.add(rol)
    const tagalma = new Discord.MessageEmbed()
        ...
        .setTimestamp()
       client.channels.cache.get("937014093674582086").send(tagalma)
  }
  })
  ...
  member.roles.add(rol)
    ...
    client.on("userUpdate", async function(oldUser, newUser) { //
        ...
        let tag2 = "Rose"; 
        let tag3 = "¿"; 
        const roleID = "937013980243849226"; 
        const guildID = "934514804494831717"; 
        const chat = "937014068634595388"; 
        const log2 = "937014093674582086"; 
        const etiket = "0105"; 
        const unregister = "937013995615965184";
